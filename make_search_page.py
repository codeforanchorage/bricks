...
<style>
 ...
 #intro { font-size: 15px; line-height: 1.5; margin: 14px 2px 0; }
 #intro p { margin: 0; }
 #nextstep { background: #e2f0d9; border-radius: 8px; padding: 9px 14px;
             font-size: 13.5px; line-height: 1.5; margin: 8px 0; }
 .grp { margin: 14px 0 2px; font-size: 15px; }
 details ul li { margin: 6px 0; }
 .buildnote { color: #888; font-size: 12.5px; margin: 10px 0 2px; }
 ...
 button.verify { margin: 2px 6px 2px 0; padding: 4px 12px; font-size: 13px;
        border: 1px solid #bcd; border-radius: 14px; background: #eef5fa;
        cursor: pointer; color: #1b4a5a; touch-action: manipulation; }
 ...
 @media (max-width: 600px) {
   ...
   button.verify { padding: 8px 14px; font-size: 14px; }
   .panel .cap { font-size: 13.5px; }
 }
__NAVCSS__</style>
</head>
<body>
...
function idLine(r) {
  ...
  const s = sec ? 'Section <span class="lt">' + esc(sec.toUpperCase()) +
                  "</span>" : "section unknown";
  ...
  for (const row of ROWS) {
    ...
    for (const qw of qwords) {
      ...
      for (const rw of row.words) {
        ...
        // And a NEAR-prefix keeps close spellings on screen while the
        // word is unfinished: score against the same-length window of
        // the candidate ("mcdonn" vs MCCONNELL's "mcconn"), slightly
        // discounted. Without this, one divergent letter mid-word made
        // the whole list collapse to 0 and pop back later (tester
        // report: mcdon -> 7 hits, mcdonn -> 0, mcdonnel -> 22).
        if (s < 0.95 && qw.length >= 4 && rw.length > qw.length) {
          const ps = ratio(qw, rw.slice(0, qw.length)) * 0.95;
          if (ps > s) s = ps;
        }
...
function run() {
  ...
  // The what-to-do-next bar rides above the results (public build only
  // -- the staff build bakes it empty).
  nextstep.style.display =
      (hits.length && nextstep.innerHTML.trim()) ? "" : "none";
}

const helpBox = document.querySelector("#wrap > details");
let qWasEmpty = true;
document.getElementById("q").addEventListener("input", () => {
  const typed = document.getElementById("q").value.trim();
  // First keystroke of a new search folds the About panel so results
  // appear where the eye already is (testers lost them below it).
  if (typed && qWasEmpty && helpBox) helpBox.open = false;
  qWasEmpty = !typed;
  clearTimeout(timer);
...
_HELP_PUBLIC = r"""<details open>
 ...
 <p class="grp"><b>Brick Search Tool - Quick Guide</b></p>
 <ul>
  <li><b>If your result shows no photo:</b>
   <ul>
    <li>The brick is still <span class="chip ok">in the official
        records</span>, but its photo was not matched.</li>
    <li>It may still be on a pallet. Bricks can be too worn to read
        or hard to match.</li>
    <li>Check your section&rsquo;s
        <a href="https://www.flickr.com/photos/anchorageparksandrec/albums">Flickr
        pallet albums</a>. If it&rsquo;s not in the albums, it is not
        at the pickup site.</li>
    <li>Which albums hold each section&rsquo;s bricks: __SECPAL__
        (a few strays landed on other pallets, so the search
        result&rsquo;s gold badge is always the surest answer).</li>
   </ul></li>
  <li><b>&ldquo;Unofficial&rdquo; result:</b>
   <ul>
    <li>The brick was found at the pickup site but is missing from
        the official lists.</li>
    <li>It can still be claimed.</li>
   </ul></li>
  <li><b><span class="chip warn">needs verification</span>:</b>
   <ul>
    <li>Two official lists disagree about this brick.</li>
   </ul></li>
  <li><b><span class="chip gray">no brick made</span> the official
      list:</b>
   <ul>
    <li>The purchase is recorded, but no brick was ever
        engraved.</li>
   </ul></li>
 </ul>
 <ul>
  <li><b>To search:</b> type the name, words you remember, or the
      original brick number. Exact spelling is not required.</li>
  <li><b>If you have the original number, enter it.</b> Some bricks
      have two numbers due to the 2009 move. The original number is
...
# brick search has its own dedicated GA4 property (Town Square
...
_PUBVERIFY = r"""
  <li><b>To see a brick photo:</b> click
      <b>show me a picture of the brick &#128247;</b>. If the photo
      does not load, the text record is still correct.</li>"""
...
def main(argv=None) -> None:
    ...
    # Staff read the stamp to confirm they have the latest upload; the
    # public subtitle does plain-English work instead, and the stats
    # move to the bottom of the About section (fresh-eyes tester found
    # "Built ... 11 unofficial" confusing as a greeting).
    stamp = ("A free volunteer tool from Code for Anchorage"
             if args.public else
    ...
    buildnote = (f"Data updated {date.today().isoformat()}: "
                 f"{len(rows):,} bricks searchable · {len(photos):,} "
                 "confirmed at the pickup site · "
                 f"{len(unofficial)} unofficial (at the site but missing "
